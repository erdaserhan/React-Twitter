...
export const deletePost = async (req, res) => {
    try {
        ...
        //We check if we're the user of the post
        if(post.user.toString() !== req.user._id.toString()) {
            return res.status(401).json({ error: "You are not authorized to delete this post"});
        }

        //We delete the image from cloudinary
        if(post.img) {
...
export const commentOnPost = async (req, res) => {
    try {
        ...
        //We push the comment in comments array
...
export const likeUnlikePost =async (req, res) => {
    try {
        const userId = req.user._id;
        const {id:postId} = req.params; //We change the name id as postId {id:postId}. 
        ...
        //We check If user liked this post already
